    ...
    <main className="simple-landing">
      <section className="simple-landing-hero fresh-hero">
        <div className="simple-hero-copy fresh-hero-copy">
          ...
          <div className="simple-hero-actions">
            ...
            <a href="#products">See today’s products <span>↓</span></a>
          </div>
      ...
      <section className="landing-shops-section">
        <div className="simple-section-heading">
          ...
          <button type="button" className="landing-view-all" onClick={() => onViewChange?.('shop-catalog')}>View all shops →</button>
        </div>
            ...
            {nearbyShops.map((shop, index) => (
              <div className="shop-card-shell" key={shop._id}>
                ...
                <div className="customer-shop-card-body">
                  ...
                  <div className="customer-shop-card-footer"><strong>{shop.distance != null ? `${shop.distance.toFixed(1)} km away` : 'Location needed'}</strong><span>View products →</span></div>
                </div>
                </button>
      ...
      <section id="products" className="simple-products-section">
        <div className="simple-section-heading"><div><span>Available now</span><h2>Products and prices</h2></div><label className="simple-product-search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products" /></label></div>
      ...
      {selectedProduct && <section id="simple-comparison" className="simple-comparison">
        <div className="simple-comparison-heading"><div><span>Price comparison</span><h2>{selectedProduct.product?.name}</h2></div><button type="button" onClick={() => setSelectedProduct(null)}>×</button></div>
